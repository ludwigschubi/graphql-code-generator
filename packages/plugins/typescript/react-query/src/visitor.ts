...
export class WebQLVisitor extends ClientSideBaseVisitor<WebQLRawPluginConfig, WebQLPluginConfig> {
  ...
  constructor(
    ...
    protected rawConfig: WebQLRawPluginConfig,
    ...
    super(schema, fragments, rawConfig, {
      ...
      reactQueryCommonImportFrom: getConfigValue(
        rawConfig.reactQueryCommonImportFrom,
        rawConfig.WebQLVersion === 2 ? '@apollo/react-common' : APOLLO_CLIENT_3_UNIFIED_PACKAGE
      ),
      reactQueryComponentsImportFrom: getConfigValue(
        rawConfig.reactQueryComponentsImportFrom,
        rawConfig.WebQLVersion === 2
          ? '@apollo/react-components'
          : `${APOLLO_CLIENT_3_UNIFIED_PACKAGE}/react/components`
      ),
      reactQueryHocImportFrom: getConfigValue(
        rawConfig.reactQueryHocImportFrom,
        rawConfig.WebQLVersion === 2 ? '@apollo/react-hoc' : `${APOLLO_CLIENT_3_UNIFIED_PACKAGE}/react/hoc`
      ),
      reactQueryHooksImportFrom: getConfigValue(
        rawConfig.reactQueryHooksImportFrom,
        rawConfig.WebQLVersion === 2 ? '@apollo/react-hooks' : APOLLO_CLIENT_3_UNIFIED_PACKAGE
      ),
      ...
      gqlImport: getConfigValue(
        rawConfig.gqlImport,
        rawConfig.WebQLVersion === 2 ? null : `${APOLLO_CLIENT_3_UNIFIED_PACKAGE}#gql`
      ),
    });
  ...
  private getreactQueryCommonIdentifier(): string {
    if (this.rawConfig.reactQueryCommonImportFrom || this.config.WebQLVersion === 2) {
      return `reactQueryCommon`;
    }

    return GROUPED_APOLLO_CLIENT_3_IDENTIFIER;
  }

  private getreactQueryHooksIdentifier(): string {
    if (this.rawConfig.reactQueryHooksImportFrom || this.config.WebQLVersion === 2) {
  ...
  private getreactQueryCommonImport(isTypeImport: boolean): string {
    const reactQueryCommonIdentifier = this.getreactQueryCommonIdentifier();

    return `${this.getImportStatement(
      isTypeImport && reactQueryCommonIdentifier !== GROUPED_APOLLO_CLIENT_3_IDENTIFIER
    )} * as ${reactQueryCommonIdentifier} from '${this.config.reactQueryCommonImportFrom}';`;
  }

  private getreactQueryComponentsImport(isTypeImport: boolean): string {
    return `${this.getImportStatement(isTypeImport)} * as reactQueryComponents from '${
      this.config.reactQueryComponentsImportFrom
    }';`;
  }

  private getreactQueryHocImport(isTypeImport: boolean): string {
    return `${this.getImportStatement(isTypeImport)} * as reactQueryHoc from '${this.config.reactQueryHocImportFrom}';`;
  }

  private getreactQueryHooksImport(isTypeImport: boolean): string {
    return `${this.getImportStatement(isTypeImport)} * as ${this.getreactQueryHooksIdentifier()} from '${
      this.config.reactQueryHooksImportFrom
    }';`;
  }
  ...
  private _buildHocProps(operationName: string, operationType: string): string {
    ...
    if (operationType === 'mutation') {
      this.imports.add(this.getreactQueryCommonImport(true));

      return `${this.getreactQueryCommonIdentifier()}.MutationFunction${typeArgs}`;
    } else {
  ...
  private _buildMutationFn(
    ...
    if (node.operation === 'mutation') {
      ...
      )} = ${this.getreactQueryCommonIdentifier()}.MutationFunction<${operationResultType}, ${operationVariablesTypes}>;`;
    }
    return null;
  }
  ...
  private _buildOperationHoc(
    ...
    this.imports.add(this.getreactQueryCommonImport(false));
 ...
 */`;
  ...
  private _buildHooks(
    ...
    this.imports.add(this.getreactQueryCommonImport(true));
    this.imports.add(this.getreactQueryHooksImport(false));

    const hookFns = [
      `export function use${operationName}(baseOptions?: ${this.getreactQueryHooksIdentifier()}.${operationType}HookOptions<${operationResultType}, ${operationVariablesTypes}>) {
        return ${this.getreactQueryHooksIdentifier()}.use${operationType}<${operationResultType}, ${operationVariablesTypes}>(${this.getDocumentNodeVariable(
        node,
        documentVariableName
      )}, baseOptions);
      }`,
    ...
    if (operationType === 'Query') {
      ...
      hookFns.push(
        `export function use${lazyOperationName}(baseOptions?: ${this.getreactQueryHooksIdentifier()}.LazyQueryHookOptions<${operationResultType}, ${operationVariablesTypes}>) {
          return ${this.getreactQueryHooksIdentifier()}.useLazyQuery<${operationResultType}, ${operationVariablesTypes}>(${this.getDocumentNodeVariable(
          node,
          documentVariableName
        )}, baseOptions);
        }`
  ...
  private _buildResultType(
    ...
    switch (node.operation) {
      case 'query':
        this.imports.add(this.getreactQueryCommonImport(true));
        return `export type ${componentResultType} = ${this.getreactQueryCommonIdentifier()}.QueryResult<${operationResultType}, ${operationVariablesTypes}>;`;
      case 'mutation':
        this.imports.add(this.getreactQueryCommonImport(true));
        return `export type ${componentResultType} = ${this.getreactQueryCommonIdentifier()}.MutationResult<${operationResultType}>;`;
      case 'subscription':
        this.imports.add(this.getreactQueryCommonImport(true));
        return `export type ${componentResultType} = ${this.getreactQueryCommonIdentifier()}.SubscriptionResult<${operationResultType}>;`;
      default:
        return '';
    }
  }
  ...
  private _buildWithMutationOptionsType(
    ...
    return `export type ${mutationOptionsType} = ${this.getreactQueryCommonIdentifier()}.BaseMutationOptions<${operationResultType}, ${operationVariablesTypes}>;`;
  }
